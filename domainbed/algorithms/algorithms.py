...
def corr_loss(x, y, device):
    vx = x - torch.mean(x, dim = -1, keepdim=True)
    vy = y - torch.mean(y, dim = -1, keepdim=True)
    cov = torch.sum(vx * vy, dim = -1, keepdim=True)
    corr = cov / (torch.sqrt(torch.sum(vx ** 2)) * torch.sqrt(torch.sum(vy ** 2)) + 1e-12)
    corr = torch.maximum(torch.minimum(corr,torch.tensor(1).to(device)), torch.tensor(-1).to(device))
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.classifier_dic = {}
        self.network_dic_init = {}
        for env_i in range(num_domains):
            self.network_dic_init[env_i] = nn.Sequential(copy.deepcopy(self.featurizer), copy.deepcopy(self.classifier))
            for param in self.network_dic_init[env_i][0].parameters():
        ...
        for i in range(num_domains):
            self.optimizer_dic[i] = torch.optim.Adam(
            self.network_dic_init[i].parameters(),
            lr=self.hparams["lr"], #  * 3
            weight_decay=self.hparams['weight_decay'])
...
class DSBH(IRM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DSBH, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
        self.num_domains = num_domains
        self.num_classes = num_classes
    ...
    def update(self, x, y, **kwargs):
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            sub_nll = F.cross_entropy(self.network_dic_init[i].to(self.device)(x), y)
            dom_loss = sub_nll
            
            self.optimizer_dic[i].zero_grad()
            dom_loss.backward()
            self.optimizer_dic[i].step()
                
            dom_logits_dic[i] = self.network_dic_init[i].to(self.device)(x)
            dom_penalty_dic[i] = self._irm_penalty(dom_logits_dic[i].detach(), y)
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            # Spuriosity Bias in terms of gradients
            inv_penalty_dic[i] = self._irm_penalty(logits, y)
            suprious_level = torch.sub((dom_penalty_dic[i]), inv_penalty_dic[i])
            grad_coef_dic[i] = torch.sqrt(torch.mean((suprious_level)**2) + 1e-9)

            # Spuriosity Bias in terms of logits
            pearson_dic[i] = corr_loss(dom_logits_dic[i], logits, self.device).detach().cpu().numpy() # cos_dist, corr_loss

        nll /= len(minibatches)
                
        grad_coef_softmax = F.softmax(torch.Tensor(list(grad_coef_dic.values())), dim=0)
        
        for i in range(len(minibatches)):
            penalty += grad_coef_softmax[i] * (inv_penalty_dic[i])
        
        inv_logits /= len(minibatches)
        pearson_softmax = F.softmax(torch.Tensor(list(pearson_dic.values())), dim=0).cuda()
        sub_logits = 0.
        for i in range(len(minibatches)):
            sub_logits += pearson_softmax[i] * dom_logits_dic[i].detach()
            
        main_sub = torch.sub(inv_logits, sub_logits)
        logit_loss = torch.sqrt(torch.mean((main_sub)**2) + 1e-9)
        
        grad_coef = self.hparams['grad_coef']
        logit_coef = self.hparams['logit_coef']
        
        loss = nll + (grad_coef * penalty) + (logit_coef * logit_loss)
        ...
        return {'loss': loss.item(), 'nll': nll.item(),
            'grad loss': (grad_coef*penalty).item(), 'logit loss': (logit_coef * logit_loss).item()}
